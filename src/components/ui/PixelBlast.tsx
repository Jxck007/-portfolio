...
const createTouchTexture = () => {
  ...
  const drawPoint = (p: TouchPoint) => {
    ...
    ctx.shadowColor = `rgba(${color},${0.28 * intensity})`;
    ctx.beginPath();
  ...
  const addTouch = (norm: { x: number; y: number }) => {
    ...
    if (last) {
      ...
      vx = dx / (d || 1);
      vy = dy / (d || 1);
      force = Math.min(Math.max(dd * 15000, 0.4), 1);
    }
...
const createLiquidEffect = (texture: THREE.Texture, opts?: { strength?: number; freq?: number }) => {
  ...
  return new Effect('LiquidEffect', fragment, {
    uniforms: new Map<string, THREE.Uniform>([
      ...
      ['uStrength', new THREE.Uniform(opts?.strength ?? 0.05)],
...
export const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  // Keep interaction methods available across renders via stable refs
  const triggerClickRipple = (clientX: number, clientY: number) => {
    const t = threeRef.current;
    const container = containerRef.current;
    if (!t || !container) return;
    const rect = container.getBoundingClientRect();
    const scaleX = t.renderer.domElement.width / (rect.width || 1);
    const scaleY = t.renderer.domElement.height / (rect.height || 1);
  ...
  const triggerPointerMove = (clientX: number, clientY: number) => {
    const t = threeRef.current;
    const container = containerRef.current;
    if (!t || !t.touch || !container) return;
    const rect = container.getBoundingClientRect();
    const nx = (clientX - rect.left) / (rect.width || 1);
    const ny = 1 - (clientY - rect.top) / (rect.height || 1);
    t.touch.addTouch({ x: nx, y: ny });
  };

  // Global window interaction listeners so interactions work everywhere on the screen
  useEffect(() => {
    const handlePointerDown = (e: MouseEvent | TouchEvent | PointerEvent) => {
      ...
      triggerPointerMove(clientX, clientY);
    };
